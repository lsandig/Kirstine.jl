...
function objective!(
    ...
    try
        ...
        for i in 1:length(pk.p)
            informationmatrix!(wm.r_x_r, wm.m_x_r, d.weight, m, wm.m_x_m, c, pk.p[i], na)
...
function gateauxderivative!(
    ...
    (; weight, designpoint) = direction
    update_model_covariate!(c[1], designpoint[1], m, cp)
    update_model_vcov!(wm.m_x_m[1], c[1], m)
    potrf!('U', wm.m_x_m[1])
    acc = 0
    for i in 1:length(pk.p)
        informationmatrix!(wm.r_x_r, wm.m_x_r, weight, m, wm.m_x_m, c, pk.p[i], na)
...
function informationmatrix(
    ...
    wm = WorkMatrices(length(weights(d)), unit_length(m), dimension(p), 1)
    for k in 1:length(c)
        Kirstine.update_model_vcov!(wm.m_x_m[k], c[k], m)
        potrf!('U', wm.m_x_m[k])
    end
    informationmatrix!(wm.r_x_r, wm.m_x_r, d.weight, m, wm.m_x_m, c, p, na)
